					...
					loff_t *ppos)
{
	...
	//hacked by zhe
	printk("[zhe]: @litmus-rt/litmus/litmus_proc.c litmus_active_proc_write is called\n");
		...
		struct proc_dir_entry** pde_in)
{
	...
	//hacked by zhe
	printk("[zhe]: @litmus-rt/litmus/litmus_proc.c proc_mkdir for C-EDF, %s, %s\n", plugs_dir, plugin->plugin_name);
					   ...
					   enum cache_level* level)
{
	...
	cluster_file = proc_create_data("cluster", 0644, parent,
					...
					(void *) level);
	//hacked by zhe
	printk("[zhe]: @litmus-rt/litmus/litmus_proc.c proc_create_data for cluster\n");
	if (!cluster_file) {
		printk(KERN_ERR
...
{
	//hacked by zhe
	printk("[zhe]: @litmus-rt/litmus/litmus_proc.c activate_domain_proc is called\n");
	int i;
	char name[8];
